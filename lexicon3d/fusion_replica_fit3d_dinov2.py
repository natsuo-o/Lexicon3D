...
def process_one_scene(data_path, out_dir, args):
    ...
    for img_id, img_dir in enumerate(tqdm(img_dirs)):
        ...
        plt.imshow(pca_feat)
        plt.savefig('room_fg.png')
        print(img_dir)
        plt.imshow(Image.open(img_dir).convert('RGB'))
        plt.savefig('origina.png')
...
def main(args):
    ...
    img_dim = (640,360) #(600,340)? (640,360)
    ...
    # intrinsic_image_dim　元画像は２倍の大きさなので*２しているよ
    intrinsic = adjust_intrinsic(intrinsic, intrinsic_image_dim=[args.img_dim[0] * 2, args.img_dim[1] * 2], image_dim=img_dim)
